 ...
 * @date 2022/7/4 14:08
 */
public class CzSpc302SWRuleBProcessFunction extends ProcessFunction<SpcCz302SWDetailDTO, SpcCz302SWDetailDTO> implements SpcProcessService {
    ...
    public void processElement(SpcCz302SWDetailDTO value, ProcessFunction<SpcCz302SWDetailDTO, SpcCz302SWDetailDTO>.Context ctx, Collector<SpcCz302SWDetailDTO> out) throws Exception {
        ...
        //获取窗口
        LimitLinkedList<SpcCz302SWDetailDTO> flexDataList = windowService.getDataWindow(value.getTaskKey(), RuleKey.RULE_B, windowKey, REDIS_FLEX_TYPE_KEY, ruleValue);
        ...
        boolean weightOk = FlinkCommonService.hasRuleBOutTag(value, ruleValue,
                ...
                () -> {
                    return flexDataList.stream().filter(s -> s.getAvg() > s.getMiddle()).count();
                },
                () -> {
                    return flexDataList.stream().filter(s -> s.getAvg() < s.getMiddle()).count();
                },

 ...
 * @date 2022/7/4 14:08
 */
public class CzSpc302ZRRuleBProcessFunction extends ProcessFunction<SpcCz302ZRDetailDTO, SpcCz302ZRDetailDTO> implements SpcProcessService {
    ...
    public void processElement(SpcCz302ZRDetailDTO value, ProcessFunction<SpcCz302ZRDetailDTO, SpcCz302ZRDetailDTO>.Context ctx, Collector<SpcCz302ZRDetailDTO> out) throws Exception {
        ...
        //获取窗口
        LimitLinkedList<SpcCz302ZRDetailDTO> flexDataList = windowService.getDataWindow(value.getTaskKey(), RuleKey.RULE_B, windowKey, REDIS_FLEX_TYPE_KEY, ruleValue);
        boolean weightOk = FlinkCommonService.hasRuleBOutTag(value, ruleValue,
                flexDataList,
                FLEX_TITLE,
                () -> {
                    return flexDataList.stream().filter(s -> s.getAvgFlex() > s.getMiddleReflexRateThreePeak()).count();
                },
                () -> {
                    return flexDataList.stream().filter(s -> s.getAvgFlex() < s.getMiddleReflexRateThreePeak()).count();
                },
        ...
        //获取窗口
        LimitLinkedList<SpcCz302ZRDetailDTO> weightDataList = windowService.getDataWindow(value.getTaskKey(), RuleKey.RULE_B, windowKey, REDIS_WEIGHT_TYPE_KEY, ruleValue);
        boolean flexOk =
                FlinkCommonService.hasRuleBOutTag(value, ruleValue,
                        weightDataList,
                        WEIGHT_TITLE,
                        () -> {
                            return weightDataList.stream().filter(s -> s.getAvgWeight() > s.getMiddleWeightThreePeak()).count();
                        },
                        () -> {
                            return weightDataList.stream().filter(s -> s.getAvgWeight() < s.getMiddleWeightThreePeak()).count();
                        },

 ...
 * @date 2022/7/4 14:08
 */
public class CzSpc302ZMRuleDProcessFunction extends ProcessFunction<SpcCz302ZMDetailDTO, SpcCz302ZMDetailDTO> implements SpcProcessService {
    ...
    public void processElement(SpcCz302ZMDetailDTO value, ProcessFunction<SpcCz302ZMDetailDTO, SpcCz302ZMDetailDTO>.Context ctx, Collector<SpcCz302ZMDetailDTO> out) throws Exception {
        ...
        //获取窗口
        LimitLinkedList<SpcCz302ZMDetailDTO> flexDataList = windowService.getDataWindow(value.getTaskKey(), RuleKey.RULE_D, windowKey, REDIS_FLEX_TYPE_KEY, ruleValue);
        ...
        boolean flexOk = FlinkCommonService.hasOutTagRuleD(value, ruleEntity.getRuleValueInt(), FLEX_TITLE,
                ...
                () -> {
                    return flexDataList.stream().map(s -> {
                        ...
                        dataAroundDTO.setTarget(s.getMiddleReflexRate());
        ...
        //获取窗口
        LimitLinkedList<SpcCz302ZMDetailDTO> weightDataList = windowService.getDataWindow(value.getTaskKey(), RuleKey.RULE_D, windowKey, REDIS_WEIGHT_TYPE_KEY, ruleValue);
        ...
        boolean weightOk = FlinkCommonService.hasOutTagRuleD(value, ruleEntity.getRuleValueInt(), WEIGHT_TITLE,
                ...
                () -> {
                    return weightDataList.stream().map(s -> {
                        DataAroundDTO dataAroundDTO = new DataAroundDTO();
                        dataAroundDTO.setSource(s.getAvgThickness());
                        dataAroundDTO.setTarget(s.getMiddleThickness());
                        return dataAroundDTO;
                    }).collect(Collectors.toList());
                },
                () -> {
